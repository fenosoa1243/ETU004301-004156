...
class DepositController extends BaseController
{
    ...
    public function store()
    {
        ...
        try {
            $transaction = $this->operationService->deposit((int) session()->get('client_id'), $montant);
        } catch (RuntimeException $e) {
            return redirect()->back()->withInput()->with('error', $e->getMessage());
        }

        return redirect()->to(site_url('client'))
            ->with('success', 'Dépôt de ' . format_money($montant) . ' effectué avec succès. Référence : ' . $transaction['reference']);
    }
}
